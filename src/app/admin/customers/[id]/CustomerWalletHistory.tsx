...
interface CustomerWalletTransactionItem {
    id: string;
    createdDate: string;
    amount: string;
    isCredit: boolean;
...
export function CustomerWalletHistory({ customerId, fallbackWalletAmount = 0 }: CustomerWalletHistoryProps) {
    const [items, setItems] = useState<CustomerWalletTransactionItem[]>([]);
    ...
    const loadHistory = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/admin/customers/${customerId}/wallet-transactions`);
            const payload = (await response.json()) as {
                data?: CustomerWalletTransactionItem[];
                stats?: CustomerWalletStats;
                error?: string;
            };
            if (!response.ok) {
        ...
        <section>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600">Ledger credits and debits for this customer.</p>
                </div>
            ...
            {!loading && !error && items.length === 0 ? (
                <div className="rounded-lg border border-gray-200 bg-white px-6 py-8 text-center text-gray-500 shadow-sm">
                    No wallet transactions for this customer.
                </div>
            ) : null}
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-600">
                                            {item.bookingServiceName ? (
                                                <div className="space-y-1">
                                                    ...
                                                    {item.bookingProviderName ? (
                                                        <p className="text-xs text-gray-500">{item.bookingProviderName}</p>
                                                    ) : null}
                                                    {item.transactionId ? (
