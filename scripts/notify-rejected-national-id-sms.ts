...
import {
    ...
    fetchRejectedNationalIdProviderIds,
} from './lib/fayda-documents';
import { loadEnvLocal } from './lib/load-env-local';
import { resolveRejectedNationalIdMessageTemplate } from './lib/rejected-national-id-sms-defaults';
...
async function main(): Promise<void> {
    ...
    const messageTemplate = resolveRejectedNationalIdMessageTemplate();
    ...
    const rejectedNationalId = await fetchRejectedNationalIdProviderIds(supabase, nationalDocIds);

    if (nationalDocIds.length === 0) {
        console.warn(
            'No documents row matched National ID / Fayda name. Check documents.name in Supabase.'
        );
    }
    ...
    if (onlyProviderId) {
        ...
        const eligible = rejectedNationalId.has(p.id);
        if (!eligible && !forceSingle) {
            console.error(
                'Provider does not have rejected National ID / Fayda verification. Use --force to send a test SMS anyway.'
            );
            process.exit(1);
        }
        if (!eligible && forceSingle) {
            console.warn(
                'Warning: provider does not have rejected National ID / Fayda verification; sending anyway (--force).'
            );
        }
        targets = [p];
    } else {
        targets = rows.filter((p) => rejectedNationalId.has(p.id));
    }

    console.log(`National-ID document types matched: ${nationalDocIds.length}`);
    console.log(`Providers (non-archived): ${rows.length}`);
    if (!onlyProviderId) {
        console.log(`Rejected National ID / Fayda: ${targets.length}`);
    } else {
        const p = targets[0];
        console.log(
            `Single provider: ${p.id}  ${providerName(p)}  rejectedNationalId=${rejectedNationalId.has(p.id)}${!rejectedNationalId.has(p.id) && forceSingle ? ' (--force)' : ''}`
        );
    }
